         ...
         (lambda (a) 
           (let (cb action label)
             (setq label  (car a)
                   ...
                   cb    `(lambda (ev) (interactive "e") 
                            (garak-notify-search-callback ev ,action)))
             (define-key garak-search-button-menu [garak-notify-search-callback]
               (list 'menu-item label cb)) ))
         (elim-avalue "buttons" args))
        (if (current-local-map)
            (use-local-map (copy-keymap (current-local-map)))
          (use-local-map (make-sparse-keymap)))
        (message "menu: %S" garak-search-button-menu)
        (local-set-key (kbd "<down-mouse-1>") garak-search-button-menu)
        ;;(message "local-map: %S" )
        (setq garak-elim-process proc))
...
(defun garak-notify-search-rows (proc name id status args)
  (let ((sid   (elim-avalue "search-id" args))
    ...
    (with-current-buffer (garak-search-results-buffer proc sid args)
      ...
      (mapc (lambda (x) (insert (apply 'format format x) "\n")) 
            (cons titles rows)) 
